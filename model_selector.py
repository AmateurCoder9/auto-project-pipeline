...
def pick_working_model(api_key: str, verbose: bool = True) -> str:
    ...
    Raises RuntimeError with a clear diagnostic message if NOTHING
    ...
    if verbose:
        print(f"[model_selector] {len(names)} candidate model(s) found, "
              f"testing in preference order...", file=sys.stderr)
    ...
    for name in names:
        ...
        if verbose:
            ...
            print(f"[model_selector]   {name}: {status} ({msg})",
                  file=sys.stderr)
        if ok:
            if verbose:
                print(f"[model_selector] Using: {name}", file=sys.stderr)
            return name
...
if __name__ == "__main__":
    ...
    if not key:
        print("Set GEMINI_API_KEY to test this module standalone.",
              file=sys.stderr)
        sys.exit(1)
    try:
        chosen = pick_working_model(key)
        print(f"\nWorking model: {chosen}")
